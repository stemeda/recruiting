...
<div>
    ...
    <div class="tab-content">
        ...
        <div role="tabpanel" class="tab-pane" id="values">
            ...
            <fieldset>
                <?php
                foreach ($positionDescription['position_description_values'] as $key => $value) {
                    echo $this->Form->input('position_description_values.' . $key . '.name', ['label' => 'Wert #' . ($key + 1), 'required' => false]);
                    echo $this->Form->input('position_description_values.' . $key . '.id', ['type' => 'hidden', 'required' => false]);
                }
        ...
        <div role="tabpanel" class="tab-pane" id="extra">
            ...
            <fieldset>
                <?php foreach ($positionDescription['position_description_extras'] as $key => $value): ?>
                    <?=$this->Form->input('position_description_extras.' . $key . '.id', ['type' => 'hidden', 'required' => false]);?>
                    Wert #<?= $key + 1 ?>
                    <?= $this->Form->input('position_description_extras.' . $key . '.settings', ['type' => 'hidden', 'data-currentid' => $key, 'class' => 'extraValueSettings']); ?>
                    <div class="row">
                        <div class="col-sm-5">
                            <?= $this->Form->input('position_description_extras.' . $key . '.name', ['required' => false]); ?>
                        </div>
                        <div class="col-sm-5">
                            <?= $this->Form->input('position_description_extras.' . $key . '.type', ['required' => false, 'empty' => 'Bitte wählen', 'data-currentid' => $key, 'class' => 'extraValueSelect', 'options' => ['bool' => 'Wahr/Falsch', 'checkbox' => 'Auswahlbox', 'text' => 'Text', 'date' => 'Datum']]); ?>
                        </div>
                        <div class="col-sm-2">
